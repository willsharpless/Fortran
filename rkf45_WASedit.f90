subroutine r4_fehl_glv ( f, neqn, y, t, h, yp, f1, f2, f3, f4, f5, s , mu, A)
...
! Will's edits to pass gLV parameters into "f"
  real ( kind = 4 ) mu(neqn)
  real ( kind = 4 ) A(neqn,neqn)

  ch = h / 4.0E+00

  f5(1:neqn) = y(1:neqn) + ch * yp(1:neqn)

  call f ( neqn, t + ch, f5, f1 , mu, A )

  ch = 3.0E+00 * h / 32.0E+00

  f5(1:neqn) = y(1:neqn) + ch * ( yp(1:neqn) + 3.0E+00 * f1(1:neqn) )

  call f ( neqn, t + 3.0E+00 * h / 8.0E+00, f5, f2, mu, A )
  ...
  call f ( neqn, t + 12.0E+00 * h / 13.0E+00, f5, f3, mu, A )
  ...
  call f ( neqn, t + h / 2.0E+00, f1, f5, mu, A )
!
...
subroutine r4_rkf45_glv ( f, neqn, y, yp, t, tout, relerr, abserr, flag, mu, A)
...
!
  if ( mflag == 1 ) then
    ...
    init = 0
    kop = 0
    call f ( neqn, t, y, yp, mu, A )  !!!! Will Edit
    nfe = 1
...
!
      call r4_fehl_glv ( f, neqn, y, t, h, yp, f1, f2, f3, f4, f5, f1, mu, A) !!!! Will Edit
      nfe = nfe + 5
!
...
subroutine r8_fehl_glv ( f, neqn, y, t, h, yp, f1, f2, f3, f4, f5, s, mu, A )
...
! Will's edits to pass gLV parameters into "f"
  ...
  call f ( neqn, t + 3.0D+00 * h / 8.0D+00, f5, f2, mu, A )
  ...
  call f ( neqn, t + 12.0D+00 * h / 13.0D+00, f5, f3, mu, A )
  ...
  call f ( neqn, t + h / 2.0D+00, f1, f5, mu, A )
!
!  Ready to compute the approximate solution at T+H.
!
  ch = h / 7618050.0D+00
...
subroutine r8_rkf45_glv ( f, neqn, y, yp, t, tout, relerr, abserr, flag, mu, A )
...
!  Advance an approximate solution over one step of length H.
!
      call r8_fehl_glv ( f, neqn, y, t, h, yp, f1, f2, f3, f4, f5, f1, mu, A )
      nfe = nfe + 5
!
!  Compute and test allowable tolerances versus local error estimates
!  and remove scaling of tolerances.  The relative error is
