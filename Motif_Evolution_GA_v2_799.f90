program Motif_Evolution_GA_v2
    ...
    integer, parameter :: S = 40, G = 300, parents = 10, seed_n = 5, choice = 2
...
!   - Instead of generating new lambda by % variations of the seed, populating with totally random values within the max and min of the corresponding NC values
    ...
    where (Lambda(:,1:N_tot) < minval(nc_params(:,1:N_tot))) Lambda(:,1:N_tot) = minval(nc_params(:,1:N_tot)) ! Ensure new mu remain within NC_param values (v2)
    where (Lambda(:,1:N_tot) > maxval(nc_params(:,1:N_tot))) Lambda(:,1:N_tot) = maxval(nc_params(:,1:N_tot))
    deallocate (r)
