...
!   - Bounded all generated values by the NC max's and min's to prevent marching
...
!   - Instead of generating new lambda by % variations of the seed, populating with totally random values within the max and min of the corresponding NC values
    ...
    print*, "Seeding with Support Community:"
    print*, seed(choice)
    ...
    ! print '(a)', ""
    ! print '(a)', "Max and Min Value of the NC mu"
    ! print *, maxval(nc_params(:,1:11))
    ! print *, minval(nc_params(:,1:11))
    ! print '(a)', ""
    ...
    enddo
    ! Bounding values to the NC params
    where (Lambda < minval(nc_params)) Lambda = minval(nc_params) ! Ensure new alpha remain within NC_param values (v2)
    where (Lambda > maxval(nc_params)) Lambda = maxval(nc_params)
    where (Lambda(:,1:N_sc) < minval(nc_params(:,1:N_tot))) Lambda(:,1:N_sc) = minval(nc_params(:,1:N_tot)) ! Ensure new mu remain within NC_param values (v2)
    where (Lambda(:,1:N_sc) > maxval(nc_params(:,1:N_tot))) Lambda(:,1:N_sc) = maxval(nc_params(:,1:N_tot))
    deallocate (r)
    ...
    do ig = 1,G
        ...
        do is = 1,S !!! Iterate through the vectors of Lambda
            ...
            do ic = 1,Total_NC !!! Iterate through the native communities
                ...
                if (.not. isnan(xfp(6,1))) then
                    ...
                    if (isnan(ra_growth(ic,1))) then
                        ...
                        if ((supported_naturally(ic,1) == 0) .and. (xfp(6,1) > 1.0E-008)) then ! old requirement (xfp(6,1)/sum(xfp) > 0.1)) >10%
                            scis = scis + 1
                        endif
                    endif
                else
            ...
            Pi(is, 1) = avg_ra_growth ! in v2, will continue to track this but irrelevant to score
            Pi(is, 2) = scis
            Pi(is, 3) = nan_penalty
            ! Pi(is, 4) = 1000*avg_ra_growth + scis + (-2)*nan_penalty - standard ! Old overall score of Lambda
            Pi(is, 4) = scis + (-2)*nan_penalty - standard ! Overall score of Lambda v2
    ...
    print *, ""
    print *, ""
end program Motif_Evolution_GA_v2
